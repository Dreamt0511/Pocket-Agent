    ...
    async def _run_executor_foreground(self, task_path: str, task_id: str):
        ...
        try:
            ...
            if needs_skill:
                ...
                if executor_step <= 2:
                    if self.ui:
                        self.ui.console.print(f"\n  [dim]📝 子Agent未执行有效操作（仅{executor_step}步），跳过技能沉淀[/dim]")
                else:
                    # 子Agent继续执行技能沉淀（使用之前的上下文，不需要重新调用大模型）
                    # 生成技能名称
                    skill_name = "".join(c if c.isalnum() or c == '_' else '_'
                                        for c in objective.lower().replace(' ', '_')
                                        .replace('（', '_').replace('）', '_')
                                        .replace('(', '_').replace(')', '_'))[:50].strip('_')
                    skill_path = f"agent/skills/auto-skills/executor/{skill_name}"

                    # 让子Agent继续执行技能沉淀
                    skill_followup = (
                        f"任务已完成。现在请沉淀本次任务的技能。\n\n"
                        f"**重要：只处理本次任务的技能，不要扫描其他目录或检查其他技能！**\n\n"
                        f"本次任务技能路径：`{skill_path}/SKILL.md`\n\n"
                        f"操作步骤：\n"
                        f"1. 用 `file_read('{skill_path}/SKILL.md')` 尝试读取\n"
                        f"2. 如果文件存在且内容完整 → 跳过，不做任何操作\n"
                        f"3. 如果文件不存在或内容不完整 → 用 `file_write` 创建/更新\n\n"
                    ...
                    # 系统检查：技能文件基本格式是否合规（只需有 frontmatter + name + description）
                    skill_file_path = os.path.join(AUTO_SKILLS_DIR, "executor", skill_name, "SKILL.md")
                    if os.path.exists(skill_file_path):
                        _format_ok = False
                        try:
                            with open(skill_file_path, "r", encoding="utf-8") as _sf:
                                _content = _sf.read(500)
                            _format_ok = (
                                _content.startswith("---")
                                and "name:" in _content
                                and "description:" in _content
                            )
                        except Exception:
                            pass
                        if not _format_ok:
                            # 格式不合规，让子Agent读 skill-creator 模板后重写
                            fix_msg = (
                                f"你沉淀的技能 {skill_name} 格式不合规。"
                                f"请先 `file_read('agent/skills/main-skills/dev/skill-creator/SKILL.md')` 读取规范格式，"
                                f"然后 `file_read('{skill_file_path}')` 读取你写的技能文件，"
                                f"最后用 `file_write` 按规范格式重写该技能文件。"
                            )
                            async for _chunk in executor_agent.astream(
